...
struct Mpz {
  ...
  explicit Mpz(unsigned long value) noexcept { __gmpz_init_set_ui(&mpz_, value); }

  void set_value(unsigned long value) noexcept { __gmpz_init_set_ui(&mpz_, value); }

  // *this = a * b
  void mul(Mpz const& a, unsigned long b) noexcept { __gmpz_mul_ui(&mpz_, &a.mpz_, b); }

  // *this *= a
  void mul(unsigned long b) noexcept { __gmpz_mul_ui(&mpz_, &mpz_, b); }

  // *this -= a * b
  void submul(Mpz const& a, unsigned long b) noexcept {
  ...
  // *this += a
  void add(unsigned long a) noexcept { __gmpz_add_ui(&mpz_, &mpz_, a); }

  // *this += a * b
  void addmul(Mpz const& a, unsigned long b) noexcept {
  ...
  unsigned long as_ui() const noexcept { return __gmpz_get_ui(&mpz_); }
...
class LFT {
 ...
 public:
  ...
  unsigned long extract(Context& ctx, unsigned long x) const noexcept {  // NOLINT
    ctx.tmp1.mul(q, x);
    ctx.tmp1.add(r);
  ...
  void produce(unsigned long n) noexcept {
    q.mul(10);
    r.submul(t, n);
    r.mul(10);
  }
